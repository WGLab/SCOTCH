...
if [ "$task" = "annotation" ]; then
    ...
    python3 $current_path/src/main_preprocessing_pacbio.py --ref $reference --bam $bam --workers $workers --target $output_dir --task annotation
...
if [ "$task" = "matrix" ]; then
  ...
  # If cover_existing is null or equals 1, run with --cover_existing
    python3 "$current_path/src/main_preprocessing_pacbio.py" --target "$output_dir" --task matrix \
...
if [ "$task" = "count" ]; then
    ...
    python3 $current_path/src/main_preprocessing_pacbio.py --target $output_dir --task count --novel_read_n $novel_read_n --workers $workers
fi
