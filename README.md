...
In general, SCOTCH accepts BAM file(s) tagged by vendor-supplied pipelines for its preprocessing pipeline (For ONT data, please see [here](https://github.com/epi2me-labs/wf-single-cell) for more details), and output a series of information including read-isoform mappings, gene count matrix, isoform count matrix, and updated gene annotations. The `--bam` argument is used to assign input file(s), and `--target` is used to assign the output folder(s). SCOTCH also supports bulk RNA-seq data, set `--bulk` argument if the input bam file is not single cell data.
...
- `--coverage_threshold_gene`: coverage threshold to support gene discovery, default is 20.
- `--min_gene_size`: inimal length of novel discovered gene, default is 50.
...
- `--save_mem_ann`: whether to use save memory mode to process in chunks, default is False
...
- `--reference_genome_fasta`: path to genome fasta file to reduce false positive novel isoform construction introduced by polyA stretch
...
- `--match_high` exon percentage upper threshold to call a read-exon mapping. For example, setting 0.6 means reads covers >60% of the exon length as mapped. (default is 0.6)
...
In this step, SCOTCH will generate a new gene annotation file, including all novel isoforms identified. Set `--task summary` to run this step. See `example/summary.sh` for an implementation in SLURM.
...
Finally, SCOTCH will generate gene- and isoform-level count matrix. Set `--task 'count matrix'` to run this step. See `example/count.sh` for an implementation in SLURM.

- `--target`: the same with step1
- `--workers`: number of threads for parallel computing.
- `--novel_read_n`: filter out novel isoforms where the number of supporting reads is less than `novel_read_n`; reads mapped to these isoforms will be treated as uncategorized. Default is 20.
- `--novel_read_pct`: filter out novel isoforms with supporting read number less than this percentage of gene coverage, default is 0.
...
- `--group_novel`: whether to group novel isoforms that are potentially generated by read truncations together as one novel isoform.
- `--platform`: 10x-ont or 10x-pacbio or parse-ont
- `--gene_subset`: optional, restrict processing to a subset of genes (see [Gene Subset Mode](#gene-subset-mode))
...
The human genome contains ~30,000 genes, and running the full pipeline can be time-consuming. For quick inspection of specific genes, SCOTCH supports a `--gene_subset` argument in the `compatible matrix` and `count matrix` steps, which restricts processing to only the specified genes.

Gene names can be provided directly on the command line:
```
python3 src/main_preprocessing.py \
--task 'compatible matrix' \
...
--reference path/to/reference/genes.gtf \
--gene_subset BRCA1 TP53 EGFR
```

Alternatively, provide a path to a plain text file with one gene name per line:
```
python3 src/main_preprocessing.py \
--task 'compatible matrix' \
--target path/to/output/folder \
--bam path/to/bam/file \
--reference path/to/reference/genes.gtf \
--gene_subset path/to/gene_list.txt
```

The same `--gene_subset` argument works for the `count matrix` step:
```
python3 src/main_preprocessing.py \
--task 'count matrix' \
--target path/to/output/folder \
