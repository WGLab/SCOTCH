    ...
    if args.task=='annotation':
        ...
        if os.path.isfile(bamInfo_pkl_file) == False and os.path.isfile(bamInfo_file) == False:
            ...
            if os.path.isfile(args.bam)==False:
                bam_info = pp.extract_bam_info_folder(args.bam, args.workers, pacbio=True)
            else:
                bam_info = pp.extract_bam_info_pacbio(args.bam)
            bam_info.to_csv(bamInfo_file)
            print('generating bam file pickle information')
            qname_dict, qname_cbumi_dict = pp.bam_info_to_dict(bam_info, False)
            with open(bamInfo_pkl_file, 'wb') as file:
                pickle.dump(qname_dict, file)
            with open(bamInfo2_pkl_file, 'wb') as file:
                pickle.dump(qname_cbumi_dict, file)
        ...
        pp.generate_compatibleVector_pacbio(args.bam, qname_dict, qname_cbumi_dict, metagene_pkl, args.match, args.target,
                                         args.job_index,args.total_jobs,args.cover_existing)
    else: # task is to generate count matrix
        adata_gene_unfiltered, adata_transcript_unfiltered, adata_gene_filtered, adata_transcript_filtered = cm.generate_count_matrix(
            path=args.target, novel_read_n=args.novel_read_n, num_cores=args.workers)
        print('count matrix generated')
        output_gene_unfiltered = os.path.join(args.target,
                                              'count_matrix/adata_gene_unfiltered' + str(args.novel_read_n) + '.csv')
        output_transcript_unfiltered = os.path.join(args.target, 'count_matrix/adata_transcript_unfiltered' + str(
            args.novel_read_n) + '.csv')
        output_gene_filtered = os.path.join(args.target,
                                            'count_matrix/adata_gene_filtered' + str(args.novel_read_n) + '.csv')
        output_transcript_filtered = os.path.join(args.target, 'count_matrix/adata_transcript_filtered' + str(
            args.novel_read_n) + '.csv')
        # save gene
        ...
        gene_meta_unfiltered = {'obs': adata_gene_unfiltered.obs.index.tolist(),
                                "var": adata_gene_unfiltered.var.index.tolist()}
        gene_meta_filtered = {'obs': adata_gene_filtered.obs.index.tolist(),
                              "var": adata_gene_filtered.var.index.tolist()}
        with open(os.path.join(args.target, 'count_matrix/adata_gene_unfiltered' + str(args.novel_read_n) + '.pickle'),
                  'wb') as f:
            pickle.dump(gene_meta_unfiltered, f)
        mmwrite(os.path.join(args.target, 'count_matrix/adata_gene_unfiltered' + str(args.novel_read_n) + '.mtx'),
                adata_gene_unfiltered.X)
        with open(os.path.join(args.target, 'count_matrix/adata_gene_filtered' + str(args.novel_read_n) + '.pickle'),
                  'wb') as f:
        ...
        transcript_meta_unfiltered = {'obs': adata_transcript_unfiltered.obs.index.tolist(),
                                      "var": adata_transcript_unfiltered.var.index.tolist()}
        mmwrite(os.path.join(args.target, 'count_matrix/adata_transcript_unfiltered' + str(args.novel_read_n) + '.mtx'),
                adata_transcript_unfiltered.X)
        transcript_meta_filtered = {'obs': adata_transcript_filtered.obs.index.tolist(),
                                    "var": adata_transcript_filtered.var.index.tolist()}
        ...
        with open(os.path.join(args.target,
                               'count_matrix/adata_transcript_unfiltered' + str(args.novel_read_n) + '.pickle'),
                  'wb') as f:
            pickle.dump(transcript_meta_unfiltered, f)
        with open(os.path.join(args.target,
                               'count_matrix/adata_transcript_filtered' + str(args.novel_read_n) + '.pickle'),
                  'wb') as f:
            pickle.dump(transcript_meta_filtered, f)
    return
