...
parser.add_argument('--key_words',type=str,default='parse_sub',help="keywords for parse sublibraries")
parser.add_argument('--sample_name',type=str,default='Sample7',help="sample name")
#optional
...
def main():
    ...
    if args.task=='annotation':
        ...
        if os.path.isfile(output) and os.path.isfile(output2):
            ...
            _ = pp.extract_annotation_info(args.ref, num_cores=args.workers, output=output,build=args.build)
        #bam information
        ...
        bamInfo3_pkl_file = os.path.join(args.target, 'bam/bam.Info3.pkl')
        ...
        if os.path.isfile(bamInfo_pkl_file) == False and os.path.isfile(bamInfo_file) == True:
            ...
            qname_dict, qname_cbumi_dict, qname_sample_dict = pp.bam_info_to_dict(bam_info, True)
            ...
            with open(bamInfo3_pkl_file, 'wb') as file:
        ...
        if os.path.isfile(bamInfo_pkl_file) == False and os.path.isfile(bamInfo_file) == False:
            ...
            if os.path.isfile(args.bam)==False:
                bam_info = pp.extract_bam_info_folder(args.bam, args.workers, parse=True)
            else:
                bam_info = pp.extract_bam_info_parse(args.bam)
        ...
        qname_sample_dict = pp.load_pickle(bamInfo3_pkl_file)
        ...
        pp.generate_compatibleVector_parse(args.bam, qname_dict, qname_cbumi_dict, qname_sample_dict, metagene_pkl, args.match, args.target,
                                         args.job_index,args.total_jobs,args.cover_existing)
    else: # task is to generate count matrix
        adata_gene_unfiltered, adata_transcript_unfiltered, adata_gene_filtered, adata_transcript_filtered = cm.generate_count_matrix_parse(path=args.target, novel_read_n=args.novel_read_n, num_cores=args.workers,key_words=args.key_words, sample_name=args.sample_name)
        print('count matrix generated')
        count_path = os.path.join(args.target, 'parse_count_matrix_' + str(args.sample_name))
        output_gene_unfiltered = os.path.join(count_path,'adata_gene_unfiltered'+str(args.novel_read_n)+'.csv')
        output_transcript_unfiltered = os.path.join(count_path,  'adata_transcript_unfiltered'+str(args.novel_read_n)+'.csv')
        output_gene_filtered = os.path.join(count_path,'adata_gene_filtered' + str(args.novel_read_n) + '.csv')
        output_transcript_filtered = os.path.join(count_path, 'adata_transcript_filtered' + str(args.novel_read_n) + '.csv')
        # save gene
        ...
        with open(os.path.join(count_path,'adata_gene_unfiltered'+str(args.novel_read_n)+'.pickle'), 'wb') as f:
            pickle.dump(gene_meta_unfiltered, f)
        mmwrite(os.path.join(count_path,'adata_gene_unfiltered'+str(args.novel_read_n)+'.mtx'),adata_gene_unfiltered.X)
        with open(os.path.join(count_path,'adata_gene_filtered'+str(args.novel_read_n)+'.pickle'), 'wb') as f:
            pickle.dump(gene_meta_filtered, f)
        mmwrite(os.path.join(count_path, 'adata_gene_filtered' + str(args.novel_read_n) + '.mtx'),
        ...
        mmwrite(os.path.join(count_path, 'adata_transcript_unfiltered' + str(args.novel_read_n) + '.mtx'), adata_transcript_unfiltered.X)
        transcript_meta_filtered = {'obs': adata_transcript_filtered.obs.index.tolist(),"var": adata_transcript_filtered.var.index.tolist()}
        mmwrite(os.path.join(count_path, 'adata_transcript_filtered' + str(args.novel_read_n) + '.mtx'),
                adata_transcript_filtered.X)
        with open(os.path.join(count_path, 'adata_transcript_unfiltered' + str(args.novel_read_n) + '.pickle'),
                  'wb') as f:pickle.dump(transcript_meta_unfiltered, f)
        with open(os.path.join(count_path, 'adata_transcript_filtered' + str(args.novel_read_n) + '.pickle'),
